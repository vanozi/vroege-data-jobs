...
class KlauwBehandeling(CreatedTimestampMixin, TimestampMixin, SQLModel, table=True):
    ...
    halsbandnummer: int = Field(
        ...
        sa_column_kwargs={"comment": "Halsbandnummer van de koe die behandeld is - koppeling naar de koe"}
    )
    ...
    notatie: str | None = Field(
        default=None,
        description="Vrije tekst notities over de behandeling (bijv. type aandoening, actie ondernomen, medicatie)",
        sa_column_kwargs={"comment": "Vrije tekst notities over de behandeling (bijv. type aandoening, actie ondernomen, medicatie)"}
    )
