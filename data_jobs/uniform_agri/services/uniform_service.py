    ...
    def get_herd_registration(
        ...
        if isinstance(raw_data, dict) and "itemList" in raw_data:
            ...
            for animal_data in animals_list:
                ...
                if "animalId" in animal_data and isinstance(
                    animal_data["animalId"], str
                ):
                    animal_data["animalId"] = UUID(animal_data["animalId"])

                # Convert birthDate from string to datetime
                if "birthDate" in animal_data and isinstance(
                    animal_data["birthDate"], str
                ):
                    v = (
                        animal_data["birthDate"].replace(".0", "")
                        if animal_data["birthDate"].endswith(".0")
                        else animal_data["birthDate"]
                    )
        ...
        if isinstance(raw_data, dict) and "animal" in raw_data:
            ...
            extracted_data = {
                **raw_data,  # Root-level properties
                ...
                "lastHerdId": UUID(animal["lastHerdId"])
                if "lastHerdId" in animal and isinstance(animal["lastHerdId"], str)
                else None,
        ...
        if isinstance(raw_data, dict) and "milkingList" in raw_data:
            ...
            for milking_data in milking_list:
                ...
                for uuid_field in ["id", "herdId", "animalId"]:
                    if uuid_field in milking_data and isinstance(
                        milking_data[uuid_field], str
                    ):
                        milking_data[uuid_field] = UUID(milking_data[uuid_field])

                # Convert datetime fields
                for datetime_field in ["shiftDate", "dateTime"]:
                    if datetime_field in milking_data and isinstance(
                        milking_data[datetime_field], str
                    ):
                        v = (
                            milking_data[datetime_field].replace(".0", "")
                            if milking_data[datetime_field].endswith(".0")
                            else milking_data[datetime_field]
                        )
